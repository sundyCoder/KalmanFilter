...
% if ~isfield(s,'A'); s.A=eye(length(3)); end
% if ~isfield(s,'B'); s.B=0; end
% if ~isfield(s,'Q'); s.Q=zeros(length(3)); end
% if ~isfield(s,'R'); error('Observation covariance missing'); end
% if ~isfield(s,'H'); s.H=eye(length(3)); end
% 
% if isnan(s.x)
...
%    s.B = [cos(s.z1(1));sin(s.z1(1));180/(s.L*pi)];
   s.B = [cos(s.z1(1));sin(s.z1(1));1/s.L];
   ...
   hx = [(s.x(1) - s.B1(1)).^2 + (s.x(2) - s.B1(2)).^2;
         (s.x(1) - s.B2(1)).^2 + (s.x(2) - s.B2(2)).^2;
         s.x(3)];
